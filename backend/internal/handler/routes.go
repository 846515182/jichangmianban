...
func RegisterRoutes(r *gin.Engine, deps *Deps) {
	...
	admin.Use(middleware.RateLimit(middleware.RateScopeAdmin), middleware.AdminAuth())
	{
		...
		admin.POST("/invite-codes", CreateInviteCode)
		admin.POST("/invite-codes/:id/disable", DisableInviteCode)
		admin.GET("/nodes", adminNodeH.NodeList)
		admin.POST("/nodes", adminNodeH.NodeCreate)
		admin.PUT("/nodes/:id", adminNodeH.NodeUpdate)
		admin.DELETE("/nodes/:id", adminNodeH.NodeDelete)
		admin.POST("/nodes/:id/rotate-token", middleware.RBAC(middleware.PermKeyManage), adminNodeH.RotateToken)
		// 一键部署涉及远程 root 权限, 仅 super_admin 可用
		admin.POST("/nodes/:id/auto-deploy", middleware.RBAC(middleware.PermNodeManage), NewAutoDeployHandler(deps.NodeRepo, deps.JWTMgr).Deploy)

		admin.GET("/users", adminUserH.UserList)
		admin.POST("/users", adminUserH.UserCreate)
		admin.PUT("/users/:id", adminUserH.UserUpdate)
		admin.DELETE("/users/:id", adminUserH.UserDelete)
		admin.POST("/users/import", adminUserH.UserImport)
		admin.POST("/users/:id/reset-traffic", adminUserH.UserResetTraffic)
		...
		admin.POST("/users/:id/status", adminUserH.UserToggleStatus)
		admin.POST("/users/:id/activate-plan", adminUserH.UserActivatePlan)
		...
		admin.PUT("/system/notify-config", systemH.UpdateNotifyConfig)
		admin.POST("/system/notify-config/test", systemH.TestNotifyConfig)

		admin.GET("/plans", planH.AdminPlanList)
		admin.POST("/plans", planH.AdminPlanCreate)
		admin.PUT("/plans/:id", planH.AdminPlanUpdate)
		admin.DELETE("/plans/:id", planH.AdminPlanDelete)

		admin.GET("/orders", orderH.AdminOrderList)
		admin.POST("/orders/:id/mark-paid", orderH.AdminMarkPaid)
		admin.POST("/orders/:id/refund", orderH.AdminRefund)
		admin.POST("/orders/:id/cancel", orderH.AdminCancelOrder)

		admin.GET("/coupons", couponH.AdminCouponList)
		admin.POST("/coupons", couponH.AdminCouponCreate)
		admin.DELETE("/coupons/:id", couponH.AdminCouponDelete)
		admin.PATCH("/coupons/:id/status", couponH.AdminCouponToggleStatus)
		...
		admin.POST("/announcements", announceAdminH.AdminCreateAnnouncement)
		admin.PUT("/announcements/:id", announceAdminH.AdminUpdateAnnouncement)
		admin.DELETE("/announcements/:id", announceAdminH.AdminDeleteAnnouncement)
		admin.PATCH("/announcements/:id/pin", announceAdminH.AdminPinAnnouncement)
		...
		admin.POST("/tickets/:id/reply", ticketH.AdminReplyTicket)
		admin.POST("/tickets/:id/close", ticketH.AdminCloseTicket)
