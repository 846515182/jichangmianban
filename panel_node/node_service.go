...
// authenticateNodeByID 统一节点鉴权(Heartbeat/GetConfig/ReportStatus 复用)。
//
// 安全加固(三处):
//  1. constant-time 比较 node_token, 防止时序攻击逐字节泄露 token。
//     旧版用 != 比较, 理论上可通过响应耗时差异逐字符爆破 token。
//  2. 节点不存在与 token 不匹配返回相同错误信息("节点凭证无效"),
//     避免攻击者通过区分 NotFound/"节点不存在" 与 Unauthenticated/"不匹配"
//     来枚举 node_id 探测节点存在性。
//  3. 统一校验顺序: 参数非空 → 查询 → token校验 → 启用状态,
//     旧版三个方法顺序各异(Heartbeat 先 IsEnabled 后 token, GetConfig 先 token 后
//     IsEnabled, ReportStatus 同 Heartbeat), 易在后续维护中引入漏洞。
//
// 注意: Register 不用此函数, 它用 GetByToken(token 查找), 本身不泄露 node_id 存在性。
func (s *NodeServiceServer) authenticateNodeByID(nodeID, nodeToken string) (*model.Node, error) {
	if nodeID == "" || nodeToken == "" {
		return nil, status.Error(codes.Unauthenticated, "节点凭证无效")
	}
	node, err := s.nodeRepo.GetByID(nodeID)
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			// 与 token 不匹配返回相同错误, 避免泄露节点存在性
			return nil, status.Error(codes.Unauthenticated, "节点凭证无效")
		}
		return nil, status.Error(codes.Internal, "查询节点失败")
	}
	// constant-time 比较, 防止时序攻击
	if subtle.ConstantTimeCompare([]byte(nodeToken), []byte(node.NodeToken)) != 1 {
		return nil, status.Error(codes.Unauthenticated, "节点凭证无效")
	}
	if !node.IsEnabled {
...
func (s *NodeServiceServer) Heartbeat(ctx context.Context, req *nexuspb.HeartbeatRequest) (*nexuspb.HeartbeatResponse, error) {
	node, err := s.authenticateNodeByID(req.GetNodeId(), req.GetNodeToken())
	if err != nil {
		return nil, err
	}
...
func (s *NodeServiceServer) ReportStatus(ctx context.Context, req *nexuspb.ReportStatusRequest) (*nexuspb.Response, error) {
	...
	if rdb == nil {
		...
		_ = s.nodeRepo.TouchOnline(node.ID, time.Now())
	...
	key := fmt.Sprintf("node:status:%s", node.ID)
	...
	_ = s.nodeRepo.TouchOnline(node.ID, now)
