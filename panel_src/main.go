...
//
// 优化(对应主机重启后 panel 崩溃循环):
// 旧版 gorm.Open 一次失败即返回, main 中 logger.Fatal 退出进程, 靠 docker
// restart 策略反复重启。当 postgres 启动稍慢(主机重启后 PG 恢复 WAL/建立连接
// 需几秒~十几秒)时, panel 会在 postgres 就绪前 fatal 退出, 触发崩溃循环,
// 且 docker 重启间隔会指数拉长(3s→15s→30s...), 恢复很慢。
// 现改为带退避的重试(最多 10 次, 间隔 2s→30s, 总等待约 2 分钟), 覆盖 PG 启动
// 慢的场景; 同时仍保留 Fatal 语义——重试耗尽才退出, 由 docker 兜底重启。
func initDB(cfg *config.Config, logger *zap.Logger) (*gorm.DB, error) {
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		// 指数退避: 2,4,6,8,10,15,20,25,30 (上限 30s)
